...
const DENY =
  /\b((civil|electrical|mechanical|hardware|structural|chemical|industrial|aerospace|biomedical|environmental|manufacturing|controls|network|optical|petroleum|process|quality|safety|materials|geotechnical|nuclear|marine|planning|water|wastewater|drainage)\s?engineer|scientist|modeller|operations\sengineer|field\sengineer|data\s?cent(er|re)|hardware|devops|sre|site\s?reliability|reliability\sengineer|security\sengineer|infosec|cyber\s?security|platform\sengineer|infrastructure\sengineer|cloud\sengineer|systems\sengineer|sales\sengineer|solutions\sengineer|support\sengineer|customer\ssuccess|implementation\s(engineer|specialist|consultant)|technical\s(recruiter|sourcer|writer|support|account)|developer\s(advocate|relations)|dev\srel|sales\sdevelopment|pre[\s-]?sales|intern(ship)?|junior|\bjr\b|staff|entry[\s-]?level|graduate|trainee|apprentice)\b/i;

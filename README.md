...
Sources: RemoteOK, Remotive, WeWorkRemotely (RSS), Hacker News "Who's hiring", Himalayas, JSearch (Google for Jobs via RapidAPI — covers LinkedIn/Indeed/Glassdoor and local boards; its search queries are configurable at runtime over MCP, including non-remote city searches), Get on Board (LatAm), Jobicy (JSON, worldwide + Europe buckets), Working Nomads (JSON, development category), Jobspresso (paged RSS, engineering listings), NoDesk (RSS + per-job page fetch for the full posting), Landing.jobs (Atom, fully-remote postings only), EU Remote Jobs (RSS, technical categories), Adzuna (official API with per-country editions; keyword/country/contract queries are configured at runtime over MCP, and the agent fetches the full posting since the API returns a snippet), plus jobs added manually over MCP and jobs extracted from email alerts.
